    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="flex-grow flex flex-col items-center justify-center pt-6">
             <h3 className="text-3xl font-black text-slate-800 mb-3 tracking-tight">{item.word}</h3>
             {item.pronunciation && (
        ...
        {/* BACK */}
        <div 
          className="absolute inset-0 w-full h-full bg-indigo-50/50 border border-indigo-100 rounded-[1.5rem] flex flex-col items-center justify-center p-6 text-left backface-hidden"
        ...
        >
           <div className="w-full h-full flex flex-col justify-center">
              <div className="mb-6 bg-white p-4 rounded-xl border border-indigo-100 shadow-sm">
                 <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">Definition</p>
                 <p className="text-slate-800 font-bold text-lg leading-snug">{item.meaning}</p>
              </div>
              
              <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">In Context</p>
                 <p className="text-slate-600 font-medium italic mb-2 text-sm">"{item.example}"</p>
                 {showTranslation && (
                     <p className="text-slate-400 text-xs font-semibold pt-2 border-t border-slate-100">{item.example_translation}</p>
                 )}
              </div>
           </div>
           
           <button onClick={handlePlayAudio} className={`absolute top-4 right-4 p-2 rounded-full hover:bg-white hover:shadow-md transition-all ${isPlaying ? 'text-indigo-600 animate-pulse' : 'text-slate-400'}`}>
                <SpeakerIcon />
           </button>
        </div>
      </div>
